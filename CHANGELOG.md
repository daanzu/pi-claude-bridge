...
- **Bump: refresh dependencies and clear npm audit findings** — require `@modelcontextprotocol/sdk` 1.30, update the pi development packages to 0.85 (including `pi-server`, now imported by `pi-coding-agent`), and refresh the lockfile.
- **Add: automated production dependency audit** — GitHub Actions checks production dependencies for high-severity vulnerabilities on pushes, pull requests, and weekly.
