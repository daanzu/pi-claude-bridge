...
- `showCost` — set to `true` to display pi-ai's model cost estimate in pi's usage footer and next to completed AskClaude results in the user-only transcript rendering, including AskClaude usage in the parent session total (default `false`).
